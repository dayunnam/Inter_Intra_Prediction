	...
	for (i = 0; i < org_col; i++)
	{
		for (j = 0; j< org_row; j++) {
			if (*(en_img + i*org_row + j) != *(de_img + i*org_row + j)) {
				//printf("%d \n", *(label + (i/ N_intra)*(org_row / N_intra) + j/ N_intra));//test 용===========
				return TRUE;
			}
		}
	}
...
int energy_f(unsigned char* org_img, unsigned char* pre_img, int size_row, int size_cul) {
	int Energy = 0;
	int v,u;
	for (v = 0; v < size_cul; v++) {
		for (u = 0; u < size_row; u++) {
			Energy += (*(org_img + v*size_row + u) - *(pre_img + v*size_row + u))* (*(org_img + v*size_row + u) - *(pre_img + v*size_row + u));
		}
	}
	return Energy;
}
